...
<html>
  ...
  <body>
  	  ...
  	  <div style="background:#F0F0F0;border:1px solid #F0F0FF;">
			<form name="order" action="${pageContext.request.contextPath}/front/order/save.action" method="post">
  	  ...
  	  <table id="table">
  			...
  			<c:forEach items="${sessionScope.bookMap}" var="entry" varStatus="idx">
  				<tr <c:if test="${idx.count%2==0}">class="even"</c:if>>
	  				...
	  				<td>
	  					${entry.value.truePrice}
	  				</td>
	  				<td>
	  					${entry.value.amt}
	  				</td>
	  				<td>
	  					<fmt:formatNumber value="${entry.value.truePrice*entry.value.amt}" pattern="#.##"/>
	  					<c:set value="${sum+(entry.value.truePrice*entry.value.amt)}" var="sum"/>
